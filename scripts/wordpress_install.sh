...
if [ ! -f .ros.provision ]; then
  echo "Name: Manual WordPress Installation (CentOS 7)" > .ros.provision
fi

name=$(grep "^Name:" .ros.provision | awk -F':' '{print $2}' | sed -e 's/^[[:space:]]*//' -e 's/[[:space:]]*$//')
if [[ "$name" != "Manual WordPress Installation (CentOS 7)" ]]; then
  echo "Current instance has already used \"$name\" tutorial configuration, cannot use this tutorial configuration again"
  exit 0
fi
...
if [[ $? -ne 0 ]]; then
  echo "Current instance cannot access public network"
  exit 0
fi
...
http {
    ...
    server {
        ...
        location ~ .php$ {
            root /usr/share/nginx/html/wordpress;    # Replace /usr/share/nginx/html with your website root directory
            fastcgi_pass 127.0.0.1:9000;   # Nginx forwards PHP requests to PHP-FPM through port 9000
            fastcgi_index index.php;
            fastcgi_param  SCRIPT_FILENAME  $document_root$fastcgi_script_name;
            include fastcgi_params;   # Nginx calls fastcgi interface to process PHP requests
        }
...
if ! grep -q "^Step6: Config MySQL$" .ros.provision; then
  ...
  mysql -uroot -e "GRANT ALL PRIVILEGES ON *.* TO 'root'@'%' IDENTIFIED BY '${db_password}'"
  echo CREATE DATABASE wordpress\; >> /tmp/setup.mysql
  echo CREATE user \"user\"@\"localhost\" identified by '\"${db_password}\"'\; >> /tmp/setup.mysql
  echo GRANT ALL privileges ON wordpress.* TO \"user\"@\"localhost\" IDENTIFIED BY '\"${db_password}\"'\; >> /tmp/setup.mysql
  echo FLUSH privileges\;>> /tmp/setup.mysql
  chmod 400 /tmp/setup.mysql
  mysql -u root --password='${db_password}' < /tmp/setup.mysql
